<template>
    <div class="MultipleTargetScoreRule">
        <div class="tip" style="padding: 10px 0"><i style="font-size: 12px;color: #f56c6c;margin-left: 10px">（tip:）提示：多对象综合训练成绩评定，适用于首长机关个人年度训练成绩、分队本级指挥员年度训练成绩、本级战术（战役）训练成绩、下一级单位年度训练成绩（本级直属队按 1 个下级单位计算成绩）评定标准。中队非战斗班人员按 1 个排（不设排的中队按 1 个班）计算；大队部除指挥员外其他人员只计个人年度训练成绩，不计入大队年度训练成绩。单课目多内容、单内容多项目训练成绩评定参照此标准执行。
        </i></div>
