<template>
  <div class="User">
    <formAndTable :schema="schema" :columns="columns" :noHandle="noHandle"></formAndTable>
  </div>
</template>
...
export default {
      ...
      schema: [
        ...
        },
        {
          ...
          onInput: (value, item) => {
            ...
            if (value !== this.schemaPassword.password)
  ...
  methods: {
    ...
    beforeSubmit(target) {
      target.organization = toPointer("Org", this.org.objectId);
    },
    beforeEdit(target) {
      target.organization =
