<template>
    <div class="soldier">
        <formAndTable :schema="schema" :columns="columns" ref="formAndTable" > </formAndTable>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                columns: [
                    ...
                    {prop: 'birthday', label: '年龄',handleValue:(value)=>this.$tools.parseTime(value.iso,'{y}年{m}月{d}日')},
                    {prop: 'politicalStatus', label: '政治面貌'},
                    {prop: 'height', label: '身高(cm)'},
                    {prop: 'weight', label: '体重(kg)'},
                    {prop: 'organization', label: '所属单位'},
                    {prop: 'position', label: '职务'},
                    {prop: 'soldierCategory', label: '人员类别'},
                    {prop: 'rank', label: '军衔等级'},
                    {prop: 'enlistedAt', label: '兵龄',handleValue:(value)=>this.$tools.parseTime(value.iso,'{y}年{m}月{d}日')},
                    {prop: 'positionAt', label: '上任日期',handleValue:(value)=>this.$tools.parseTime(value.iso,'{y}年{m}月{d}日')},
                ],
                schema: [
                    ...
                    {fieldType: "autocomplete",fetchSuggestions: this.querySearch,placeholder: "单位名称",label: "单位名称",vModel: "organization",organization:'',required: true,
                        onChange:(obj,value,found)=>{
                            ...
                            this.editOrg = value||{}
                            queryListByKeyValue('Position',{orgSequence:value.orgSequence,sorts:{ascending:'sortCode'}}).then(list=>{
        ...
        created() {
            if(this.$store.getters.organization.objectId)this.equalTo = {
                organization:toPointer('Org',this.$store.getters.organization.objectId)
            }
        ...
        methods:{
            ...
            beforeSubmit(target) {
                target.organization = toPointer("Org", this.editOrg.objectId);
