<template>
    <div class="Report">
        <div class="body">
                ...
                <div class="title-text"  v-if="tableData">
                    ...
                    <div class="text-btns">
                        <div>
                            ...
                            <el-button type="primary" v-if="!patternIsEdit && $route.path !== '/TableVue'" class="print-hidden" @click="$units.showTable(tableData,'MonthlyReportL1')" size="mini" plain title="新窗口打开">新窗口打开（打印）</el-button>
                        </div>
            ...
            <div class="body-content" v-if="tableData">
                ...
                <div class="content-table">
                    <table border="1" cellspacing="0" cellpadding="0" class="table-height">
                        ...
                        <tr v-for="(item,index) in tableData.timeStat">
                            <td>{{item.type|handlePersonProperty}}</td>
                            <td>{{item.hoursInDay}}</td>
                            <td>{{item.hoursAtNight}}</td>
                            <td>{{item.totalHoursInDay}}</td>
                            <td>{{item.totalHoursAtNight}}</td>

                            <td v-if="index === 0" class="td-zindex" :rowspan="tableData.timeStat.length">
                                <input type="number" :value="tableData.qualityStat.excellentRate|handleZero" @input="handlerInput('qualityStat','excellentRate',$event)" :readonly="!patternIsEdit">
                            </td>
                            <td v-if="index === 0" class="td-zindex" :rowspan="tableData.timeStat.length">
                                <input type="number" :value="tableData.qualityStat.goodRate|handleZero" @input="handlerInput('qualityStat','goodRate',$event)" :readonly="!patternIsEdit">
                            </td>
                            <td colspan="2" v-if="index === 0" class="td-zindex" :rowspan="tableData.timeStat.length">
                                <input type="number" :value="tableData.qualityStat.passRate|handleZero" @input="handlerInput('qualityStat','passRate',$event)" :readonly="!patternIsEdit">
                            </td>
                            <td v-if="index === 0" class="td-zindex font-family" :rowspan="tableData.timeStat.length">
                        ...
                        <tr>
                            <td class="td-zindex" height="50">
                                <input type="number" :value="tableData.trainerStatistic.officerCount|handleZero" @input="handlerInput('trainerStatistic','officerCount',$event,true)" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex">
                                <input type="number" :value="tableData.trainerStatistic.monitorCount|handleZero" @input="handlerInput('trainerStatistic','monitorCount',$event,true)" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex">
                                <input type="number" :value="tableData.trainerStatistic.otherCount|handleZero" @input="handlerInput('trainerStatistic','otherCount',$event,true)" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex">
                                <input type="number" :value="tableData.trainerStatistic.standardCount|handleZero" @input="handlerInput('trainerStatistic','standardCount',$event,true)" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex">
                                <input type="number" :value="tableData.trainerStatistic.standardRate|handleZero" @input="handlerInput('trainerStatistic','standardRate',$event)" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex font-family">
                                <input v-model="tableData.placeStatistic.created" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex font-family">
                                <input v-model="tableData.placeStatistic.nonCreated" :readonly="!patternIsEdit">
                            </td>
                            <td colspan="2" class="td-zindex font-family">
                                <input v-model="tableData.placeStatistic.newCreated" :readonly="!patternIsEdit">
                            </td>
                            <td class="td-zindex">
                                <input type="number" :value="tableData.placeStatistic.passRate|handleZero" @input="handlerInput('placeStatistic','passRate',$event)" :readonly="!patternIsEdit">
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr v-for="index in bulletStatLength">
                            <td colspan="3" class="font-family">{{(tableData.motorStat[index-1]||{}).type || '　'}}</td>
                            <td class="font-family">{{(tableData.motorStat[index-1]||{}).count}}{{(tableData.motorStat[index-1]||{}).unitType}}</td>
                            <td class="font-family">{{(tableData.motorStat[index-1]||{}).total}}{{(tableData.motorStat[index-1]||{}).unitType}}</td>

                            <td class="font-family">{{(tableData.bulletStat[index-1]||{}).type}}</td>
            ...
            <p v-else style="text-align: center;color: #999"><i class="el-icon-warning"></i> 登记该月份的日登记表，将自动生成</p>
...
<script>
    ...
    import {PersonProperty,ScoreCriteria2} from 'src/lib/Constants'
    export default{
        props:{
            tableData:{
                type:Object,
        ...
        computed:{
            bulletStatLength(){
                return this.tableData.bulletStat.length > 9 ? this.tableData.bulletStat.length : 9
            },
            Level4(){
