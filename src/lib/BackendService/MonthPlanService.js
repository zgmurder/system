...
export class MonthPlanService {
    constructor(backend) {
        ...
        this.annualPlanItems = undefined;
        this.stagePlanItems = [];
    ...
    async getMonthPlanDetails(objectId) {
        try {
            ...
            await this._fetchPlanItems(monthPlan, monthPlan.year, monthPlan.month, monthPlan.organization);
    ...
    updateArrangedCourse({planItem, courseItem, hoursInDay=0, hoursAtNight=0, isOfficer=false, isAdd=true}) {
        ...
        if (isAdd) {
            ...
            if (isOfficer) {
                this.monthPlan.officerHours += (hoursInDay+hoursAtNight);
            }
            ...
            if (!_.isEmpty(removed)) {
                ...
                if (isOfficer) {
                    this.monthPlan.officerHours -= (removedCourse.hoursInDay+removedCourse.hoursAtNight);
                }
    ...
    async _fetchPlanItems(annualPlan, year, month, organization) {
        // 先获取阶段计划
        let monthBegin = moment(new Date(year, month-1, 1)).startOf('month').toDate();
        let monthEnd = moment(new Date(year, month-1, 1)).endOf('month').toDate();
        ...
        this.courseTimeStatuses = result.list;
    }
