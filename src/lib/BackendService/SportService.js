...
export class SportService {
    ...
    async deleteSportEvent(assessEvent) {
        try {
            ...
            if (assessEvent.assessMethod === AssessMethod.Sampling) {
                if (currOrg && assessEvent.organizer && assessEvent.organizer.objectId != currOrg.objectId) throw '无权限删除其他单位创建的体育成绩登记表';
            } else {
    ...
    async addOrUpdateSportEvent(assessEvent) {
        try {
            ...
            if (!assessEvent.objectId) {
                sportScores = sportScores.map(item => {
                    item.assessEvent = updatedEvent;
                    return item;
                });
            }
