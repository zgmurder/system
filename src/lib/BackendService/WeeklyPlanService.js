...
export class WeeklyPlanService {
    ...
    async getWeeklyPlanDetailsByObjectId(objectId) {
        try {
            ...
            if (weeklyPlan.state != ApproveState.Approved) {
                let today = this.backendService.getSystemTime();
                let currOrg = this.backendService.getCurrentOrganization();
    ...
    async editWeeklyPlan(weeklyPlan) {
        try {
            ...
            // let theSameWeek = false;
            // if (!_.isEmpty(this.weeklyPlan)) {
            //     if (this.weeklyPlan.year===weeklyPlan.year && this.weeklyPlan.month===weeklyPlan.month && this.weeklyPlan.weekSeq===weeklyPlan.weekSeq) {
            //         theSameWeek = true;
            //     }
            // }
            ...
            if (!(today >= weeklyPlan.fromDate && today <= weeklyPlan.toDate) && !(nextWeekDate >= weeklyPlan.fromDate && nextWeekDate <= weeklyPlan.toDate)) throw '只能编辑本周或下周工作安排表';

            // 本周的情况下
            if (today >= weeklyPlan.fromDate && today <= weeklyPlan.toDate) {
                nextWeekDate = today;
                today = moment(nextWeekDate).subtract(7, 'days').toDate();
            }

            this.weekOfMonth = dateUtils.weekOfMonth(today);
            this.nextWeekOfMonth = dateUtils.weekOfMonth(nextWeekDate);
    ...
    async addOrUpdateWeeklyPlan() {
        try {
            ...
            let result = await this.backendService.addOrUpdateList('DailyLesson', dailyLessons);
            ...
            result = await this.backendService.addOrUpdateList('CourseStatus', this.updatedCourseStatuses);
    ...
    async _initWeeklyPlan() {
        ...
        this.weeklyPlan = {
            ...
            serviceReq: this.organization.serviceReq,
    ...
    async _fetchRules(organization, theSameWeek=false) {
        ...
        if (_.isEmpty(this.monthPlanItem)||!theSameWeek) {
            ...
            if (!_.isEmpty(organization.serviceReq)) {
                query.equalTo('serviceReq', organization.serviceReq);
            }
            query.equalTo('year', this.nextWeekOfMonth.year);
        ...
        if (_.isEmpty(this.lastWeeklyPlan)||!theSameWeek) {
            ...
            if (this.organization.orgSequence > OrgSequence.Company) {
                ...
                if (_.isEmpty(this.mainWeeklyPlan)) throw `请先创建主单位的下周工作安排表`;
            }
        }
