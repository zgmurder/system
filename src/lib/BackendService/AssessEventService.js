...
export class AssessEventService {
        ...
        try {
            ...
            if (assessMethod === AssessMethod.Normal) {
                ...
                if (existed) {
                    ...
                    // query.equalTo('assessEvent', this.backendService.getParseObject('AssessEvent', existed.objectId));
                    // let result = await this.backendService.queryListAll('PersonScore', query);
                    // existedPersonScores = result.list;
                }
            }
    ...
    async addOrUpdateAssessEvent(assessEvent) {
        try {
            ...
            // let personScoreGroups = assessEvent.personScoreGroups;
            // delete assessEvent.personScoreGroups;
    ...
    async deleteAssessEvent(assessEvent) {
        try {
            ...
            if (assessEvent.assessMethod === AssessMethod.Sampling) {
                if (currOrg && assessEvent.organizer && assessEvent.organizer.objectId != currOrg.objectId) throw '无权限删除其他单位创建的军事成绩登记表';
            } else {
    ...
    _statLevelCount(personScores) {
        let filtered = personScores.filter(item => !(item.score === 0 && item.makeupScore === 0));
    ...
    async _evaluateMasterPersonScore(masterScore, personScores) {
        ...
        if (_.isEmpty(this.multipleTargetScoreRules)) {
            let standardQuery = new Client.Query(Client.TrainStandard);
            standardQuery.equalTo('state', StandardState.Using);
            standardQuery.descending('score')
