...
import { DailySection,CourseCategory,OrgSequence, SubmitState, InServiceStatus, DayType } from '../Constants';
            ...
            if (!_.isEmpty(list)) {
                ...
                planLessons.forEach(ele => {
                    ...
                    if(ele.section === DailySection.EarlyMoring)
                        ...
                        ele.courses.forEach(item =>  report.contentStat.morning.push(item.name));
                    else if(ele.section === DailySection.Afternoon)
                        ele.courses.forEach(item =>  report.contentStat.afternoon.push(item.name));
                    else if(ele.section === DailySection.Afternoon)
                        ele.courses.forEach(item =>  report.contentStat.sport.push(item.name));
            ...
            if (!report) {
                ...
                report.bulletStat.map(item => {
                    if (item.trainAverage) {
                        item.soldierNumber = Math.round(Number((Number(item.train) / Number(item.trainAverage)).toFixed(1)))
                    }
                });
