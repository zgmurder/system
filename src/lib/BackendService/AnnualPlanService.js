...
export class AnnualPlanService {
    ...
    async addOrUpdateAnnualPlanSummary(summary) {
        try {
            ...
            let results = await this.backendService.addOrUpdateList('AnnualStage', annualStages);
    ...
    async getInitOfficeAnnualPlan(year, organization, orgCategory) {
        try {
            ...
            query.lessThanOrEqualTo('year', year);
            query.descending('year').limit(1);
            result = await this.backendService.queryList('OfficeAnnualPlan', query);
            if (_.isEmpty(result.list)) {
                // 初始化表数据
                ...
                plan = result.list[0];
                if (plan.year === year) throw `${plan.name}只能创建一张不能重复创建`;

                // 直接复制之前已经创建的表数据
                delete plan.objectId;
                plan.year = year;
                plan.name = planName;
                plan.state = SubmitState.Initial;
            }
        ...
        try {
            ...
            // if (_.isEmpty(result.list))
            {
                ...
                annualPlan.planItems = annualStage.stages.map(stage => {
                    // let days = moment(stage.toDate).diff(moment(stage.fromDate), 'days');
    ...
    async addOrUpdateUnitForceAnnualPlan(annualPlan) {
        try {
            ...
            let results = await this.backendService.addOrUpdateList('UnitForceAnnualPlanItem', planItems);
