...
export class RegimentReportService {
    ...
    async judgeOrgAndDateIsOk(organization, date, timeoutFlag) {
        try {
            ...
            // 获取支队下需提交月统计表的分队总数
            let monthlyL1ReportInfo = await this.backendService.reportCommonApi.getAllUnitForcebyParentId(organization.objectId);
    ...
    async updateAllMonthReport(org,year) {
        try {
            ...
            query.lessThan('date', moment().endOf("year").toDate());
            ...
            if(result.total) {
                ...
                for(let monthReport of sortedReports) {
                    ...
                    // 获取支队下需提交月统计表的分队总数
                    let result = await this.backendService.reportCommonApi.getAllUnitForcebyParentId(orgId);
                    // 获取已提交的所有分队月统计表
                    let tableList = [];
