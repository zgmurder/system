...
export class ReportCommonApi {
        ...
        try {
            ...
            result.bulletStat.map(item => {
                ...
                if (item.trainAverage > 0)
                    item.soldierNumber = Math.round(Number((item.train / item.trainAverage).toFixed(1)));
            });
            // 查询中队最近的月统计表数据
    ...
    async getAllUnitForcebyParentId(orgId) {
        try {
            ...
            orgQuery.equalTo('orgSequence', OrgSequence.Company);

            let results = await this.backendService.queryListAll('Organization', orgQuery);
            return results;
            // let orgListAll = [];
            // for (let org of results.list) {
            //     if (org.orgSequence === OrgSequence.Company) {
            //         orgListAll.push(org);
            //         let list1 = await this.backendService.orgService.getLocalOrganizations(org);
            //         let list2 = [];
            //         list2 = list1.filter(item => item.orgCategory !== org.orgCategory && item.orgSequence > org.orgSequence);
            //         orgListAll = orgListAll.concat(list2);
            //     } else if (org.orgSequence === OrgSequence.Battalion) {
            //         let companyOrgQuery = new Client.Query(Client.Organization);
            //         companyOrgQuery.equalTo('parentId', org.objectId);
            //         let companyOrg = await this.backendService.queryListAll('Organization', companyOrgQuery);
            //         if (companyOrg.list) {
            //             for (let company of companyOrg.list) {
            //                 orgListAll.push(company);
            //                 let list3 = await this.backendService.orgService.getLocalOrganizations(company);
            //                 let list4 = [];
            //                 list4 = list3.filter(item => item.orgCategory !== company.orgCategory && item.orgSequence > company.orgSequence);
            //                 orgListAll = orgListAll.concat(list4);
            //             }
            //         }
            //     }
            // }
            // return {total: orgListAll.length, list: orgListAll }
        } catch (exception) {
    ...
    getBulletStatByMonthlyL2Reports(monthlyL2Reports){
        ...
        for(let key in groupedObject) {
            ...
            let bulletStat = bulletTypes.map(type => {
                ...
                if (result.trainAverage > 0) result.soldierNumber += +(result.train / result.trainAverage).toFixed(2);
                return result;
            });
            ...
            if (pistolIndex >= 0) {
                ...
                if (bulletStat[pistolIndex].trainAverage > 0) bulletStat[pistolIndex].soldierNumber = +(bulletStat[pistolIndex].train / bulletStat[pistolIndex].trainAverage).toFixed(2);
                if (bulletStat[pistolIndex].totalTrainAverage > 0) bulletStat[pistolIndex].totalSoldierNumber = +(bulletStat[pistolIndex].totalTrain / bulletStat[pistolIndex].totalTrainAverage).toFixed(2);
            }
    ...
    getBulletStatByMonthlyL1Reports(lastReportInfo, monthlyL1Reports){
        ...
        let bulletStat = bulletTypes.map(type => {
            let result = monthlyL1Reports.reduce((prev, curr) => {
                ...
                if (stat) {
                    ...
                    prev.train += stat.train;
                    prev.others += stat.others;
                    if (stat.totalTrainAverage>0) prev.totalSoldierNumber += +(stat.totalTrain/stat.totalTrainAverage).toFixed(2);
                    prev.totalTrain += stat.totalTrain;
                    prev.totalOthers += stat.totalOthers;
                }
                return prev;
        ...
        console.log('getBulletStatByMonthlyL1Reports', bulletStat);
        return bulletStat;
    }
    ...
    async getOfficeStatistic(lastReportInfo, org, date){
        try{
            ...
            if(result.total) {
                ...
                // let soldiers = await this.backendService.soldierService.getAllLocalSoldiers(org);
                // ret.bulletStatOffice.trainAverage = Math.round(ret.bulletStatOffice.train / soldiers.length);
